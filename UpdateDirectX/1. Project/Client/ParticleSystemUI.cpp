...
void ParticleSystemUI::update()
{
	if (nullptr != GetTarget())
	{
		...
		m_pMaterrial = Target->GetCurMaterial();
	}
	ComponentUI::update();
}
...
void ParticleSystemUI::SetTexture(DWORD_PTR _strMaterialKey)
{
	...
	Target->GetCurMaterial()->SetTexParam(TEX_0, CResMgr::GetInst()->FindRes<CTexture>(wstrKey));
}
...
void ParticleSystemUI::SetMaterial(DWORD_PTR _strMaterialKey)
{
	...
	Target->SetSharedMaterial(CResMgr::GetInst()->FindRes<CMaterial>(wstrKey));
}
