#ifndef _BLOODPARTICLE
#define _BLOODPARTICLE
...
[numthreads(128, 1, 1)]
void CS_BloodParticle(uint3 _id : SV_DispatchThreadID)
{
    ...
    else
    {
        ...
        {
            float2 Accel = float2(100, 800);
            float2 gravity = float2(0, 4500);
            
            float2 velocity = float2(0,0);
            ShareData.vVelopcity = float2(0, -20.f);
            
            
            float2 Gravity = gravity;
            Gravity = normalize(Gravity);
            Gravity = Gravity * dot(ShareData.vVelopcity, Gravity);
            float2 vUV = float2((float) _id.x / PARTICLE_MAX, 0.5f);
            float4 vRandom = float4(GaussianSample(vUV + float2(0.f, 0.f)).x, GaussianSample(vUV + float2(0.1f, 0.f)).x, GaussianSample(vUV + float2(0.2f, 0.f)).x, GaussianSample(vUV + float2(0.3f, 0.f)).x);
            ShareData.vForce.x = vRandom.z * (MaxSpeed - MinSpeed) + MinSpeed;
            

            ShareData.vVelopcity += ShareData.vForce;
            ShareData.vVelopcity.x += Gravity.x;
            ShareData.vVelopcity.y -= Gravity.y;
            
            if (ShareData.vVelopcity.y < -500)
                ShareData.vVelopcity.y = -500;
            
            Particle.vRelativePos.x += ShareData.vVelopcity.x * g_fDT;
            Particle.vRelativePos.y += ShareData.vVelopcity.y * g_fDT;
        }
    }
}
