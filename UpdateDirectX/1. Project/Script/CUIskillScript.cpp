...
void CUIskillScript::tick()
{
	...
	MeshRender()->GetCurMaterial()->SetScalarParam(SCALAR_PARAM::INT_0, &m_iSpark);
	...
	if (KEY_PRESSED(KEY::LSHIFT) || KEY_TAP(KEY::LSHIFT))
	{
		if (m_fAccSkill > 0.5f)
		{
			...
			if (m_iIdx != -1)
			{
				m_pSkilltime[m_iIdx]->MeshRender()->GetDynamicMaterial()->SetScalarParam(SCALAR_PARAM::INT_1, &a);
				m_fAccSkill = 0;
