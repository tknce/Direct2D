...
void CGrountScript::tick()
{
    ...
    if (KEY_PRESSED(KEY::LSHIFT))
        ...
        m_iSkill = 0;
    GetOwner()->MeshRender()->GetCurMaterial()->SetScalarParam(INT_0, &m_iSkill);
...
void CGrountScript::DEAD()
{
    if (m_StateValue.enter == true)
    {
        ...
        if (m_pParticle == nullptr)
        {
            ...
            m_pParticle->ParticleSystem()->GetCurMaterial()->SetTexParam(TEX_0, CResMgr::GetInst()->FindRes<CTexture>(L"texture\\particle\\Blood.png"));
            Vec3 Pos = Transform()->GetRelativePos();
            //Pos.z = 0;
            Pos.y -= 50;
