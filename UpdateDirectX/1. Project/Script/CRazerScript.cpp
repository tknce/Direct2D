	...
	if (_pOther->GetOwner()->GetName() == L"Player")
	{
		if (CPlayerMgr::GetInst()->GetDead() == 0 && _pOther->GetOwner()->GetScript<CPlayerScript>()->GetPower() > 0.5f)
		{
			...
			if (m_pParticle == nullptr)
			{
				...
				m_pParticle->ParticleSystem()->SetSharedMaterial(CResMgr::GetInst()->FindRes<CMaterial>(L"ParticleRenderMtrl"));
				m_pParticle->ParticleSystem()->GetCurMaterial()->SetTexParam(TEX_0, CResMgr::GetInst()->FindRes<CTexture>(L"texture\\particle\\Sparks.png"));
				//Razer->SetName(L"1");
			...
			_pOther->GetOwner()->MeshRender()->GetDynamicMaterial()->SetScalarParam(INT_1, &a);
			//CPlayerMgr::GetInst()->SetDead(1);
		}
		else
		{
...
void CRazerScript::Overlap(CCollider2D* _pOther)
{
	if (_pOther->GetOwner()->GetName() == L"Player")
	{
		if (CPlayerMgr::GetInst()->GetDead() == 1)
		{
			...
			_pOther->GetOwner()->MeshRender()->GetDynamicMaterial()->SetScalarParam(FLOAT_3, &m_fRazer);
		}
		else
		{
			m_fRazer = 0;
